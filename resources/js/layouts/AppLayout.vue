...
const visibleMenuItems = computed(() => {
    const roleName = user.value?.role?.nama_role;
    return menuItems.filter((item) => !item.roles || item.roles.includes(roleName));
});
                ...
                <template v-for="item in visibleMenuItems" :key="item.label">
                    <div v-if="!item.items">
                        <Link
                            ...
                            :class="$page.url.startsWith('/' + item.route.split('.')[0]) ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-100'"
                        >
                    ...
                    <div v-else class="mb-2">
                        ...
                        <div v-for="sub in item.items" :key="sub.label">
                            <Link
                                ...
                                class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors"
                                :class="route.current(sub.route) ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-600 hover:bg-slate-100'"
                            >
